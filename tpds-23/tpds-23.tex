...
minute, the peers need to roll back only 3\% of the time to remain fully
symmetric.
\end{abstract}
...
%
This problem relates to consistency in distributed systems, and is also known
as \emph{state melding}~\cite{melding} in the context of virtual worlds, which
adds the requirement of a time deadline to reach consistency.
...
%
Our middleware provides \emph{perceptive consistency}~\cite{melding}, which
ensures that an event triggered by any peer at time $t$ is perceived by all
peers simultaneously at time $t+\Delta$.
%
...
We also that assume peers are non malicious in the sense that they do not lie
...
As it is the case for the discussed work, we consider that middlewares for
symmetric applications, which provide bit-identical behavior, must comply with
\emph{perceptive consistency}~\cite{melding}, as follows:
...
%
Automerge~\cite{p2p.automerge} along with its accompanying
Hypermerge~\cite{p2p.pushpin} peer-to-peer infrastructure is an example of
decentralized CRDTs.
...
previous section.
Nevertheless, programmers can still use CRDTs on top of a middleware, since
they are nothing more than data structures and merging algorithms.

CRDTs provide \emph{delayed consistency}~\cite{melding}, as follows:
...
scheduled to the future, such that all peers can satisfy (also known as
\emph{local lag}~\cite{mauve}]).
...
never lie about their local clocks, which would affect the event timestamps,
allowing a peer to modify the global timeline with nonsensical information.
%
For this reason, we assume a permissioned peer-to-peer network with an external
...
Note that using callbacks is a already common technique for event-driven
...
\code{cb\_ren}~\cite{tml.js} must be separated from \code{cb\_sim}, which would
...
The callback \code{cb\_evt} is invoked by the middleware in only real time,
...
In our example, we only generate application events for the 5 key of interest,
...
\begin{comment}
- O design experimental precisa ser melhor descrito.
Apenas o emulador das latências é descrito (NetEm).
Como foi realizada (ou qual ferramenta foi usada) a simulação dos experimentos?
\end{comment}
...
This means that an application using the middleware would perform all network
synchronization and still have almost 100\% of the CPU time available for its
actual domain logic.
%
...
%
%To our knowledge, it is the first middleware to support bit-identical real-time
%behavior in decentralized peer-to-peer networks.
%
...
adds a time deadline to reach consistency, ensuring that an event triggered by
any peer at time $t$ is perceived by all peers simultaneously at time
$t+\Delta$.
