...
a single machine.
%
In this work, we propose a middleware for these \emph{symmetric peer-to-peer
...
%
We show that the middleware can handle applications with over 20 peers in
...
\label{sec.introduction}
 ...
 6. ***How does the middleware handle consistency when advancing or rollbacking in  time to synchronize?***
About #6, this is a valid criticism and we should include a brief discussion about consistency models and position our work on this spectrum.
The facts that peers share a global clock and go through bit-identical steps may characterize strict consistency.
However, since the global clocks may diverge temporarily, we must rely on rollbacks.
The key to preserve consistency in the presence of rollbacks is to restrict our programming API to deterministic and stateless calls only.
This way, it is not possible to diverge from other peers, for example, by re-executing stateful file operations multiple times (or at different times).

&gt; A apresentação da solução entra em um nível de detalhamento muito baixo...
  ...na decisão da topologia utilizada na simulação não fica muito claro se  essa topologia representa algum cenário específico.

Sobre a topologia escolhida, optamos por misturar 3 arranjos abstratos:

1. linha contínua
2. ciclo fracamente conectado
3. ciclo totalmente conectado

Entendemos que casos concretos que sejam subconjuntos dos arranjos acima terão desempenho igual ou superior ao exibido na Figura 7.
Por exemplo, uma linha contínua de até 15 nós é um subconjunto da topologia escolhida, assim como dois ciclos conectados separados por um nó.
Assim, pretendemos atender a um número maior de cenários, mesmo que isso dependa de uma interpretação mais apurada por parte dos interessados.
De qualquer maneira, a escolha é arbitrária e não representa um cenário concreto.

&gt; Figures 1 and 2 create an expectation of approaches comparison; I suggest writing in the motivation why there will be no clear comparison in the
  experimental results.
We will be more explicit about this in Sections 2/4, since it is not possible to reproduce our experiment in centralized solutions (the topology is inherently peer-to-peer).
We only discuss the performance of related works based on their claims and also on theoretical upper limits."

Os autores mencionam que em cenário com latência de 50ms e 25 eventos por minuto, os pares precisaram retroceder 3\% do tempo. Eles devem deixar claro porque isso é uma vantagem e qual a média de período de tempo aceitável para retrocesso.
- Nos trabalhos relacionados, apenas propostas centralizadas são mencionadas e comparadas. Não há propostas descentralizadas para comparação também? Sugiro expandir essa seção.
- O design experimental precisa ser melhor descrito. Apenas o emulador das latências é descrito (NetEm). Como foi realizada (ou qual ferramenta foi usada) a simulação dos experimentos?
- Apenas aplicações com deadline mais curtos adequam-se melhor ao middleware.  Os autores precisam explicar no texto os resultados dos experimentos com maior latência também (100ms, 250ms e 500ms).
- Os autores precisam deixar claro o benefício de usar uma solução descentralizada desde que o seu desempenho é menor que as soluções centralizadas.
...
local-first
Apesar dos rollbacks quando a rede se reconecta serem inevitáveis, os seus efeitos podem ser imperceptíveis para os usuários finais (e.g., quando há pouca
atividade ou dependência entre eventos).

O principal benefício é que a aplicação continua funcionando mesmo com a
presença de partições na rede, o que inclui a queda do (que seria o) servidor
em uma solução centralizada.

Levando essa possibilidade ao extremo, um peer ainda poderia funcionar
normalmente mesmo que totalmente offline por um determinado período (como é
comum em pesquisas recentes em &quot;local-first software&quot;).
