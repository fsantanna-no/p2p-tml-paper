...
collaborative documents, watch parties, and multi-player games~\cite{TODO:collab}.
%
...
networks~\cite{TODO:p2p}.
Events are flooded in the graph and are triggered with a small deadline delay
...
As an example, in a scenario with a network latency of $50ms$ and 25 events per
...
bit-identical steps, but they do eventually reach the same
state~\cite{TODO:eventual}.
%
...
and do not support real-time consensus among peers~\cite{TODO:consensus}.
%
...
%
Regarding decentralized machines, Fusion~\cite{fusion} is a state
...
The middleware assumes that each peer assigns itself a unique identifier, such
as TODO~\cite{TODO:hash}.
...
The middleware renders the screen only after each frame in real time.
...
peers with a timestamp scheduled to the future with a deadline $\Delta$, such
that all peers are able to apply them in sync.
%
To prevent broadcast cycles, each peer keeps a vector of the highest timestamps
it received from each other peer, such that lower numbers can be ignored when
received.
...
how it ensures the desired properties in concrete terms as follows:
    (a) peers meet event deadlines either travelling back and forth or in real
        time (\linx{12--16} or \linx{18--22} / \ref{sec.tml.middleware.loop});
...
We found an overhead of $9us$ in the average, which corresponds to less than
$1/2000$ of the CPU time.
%
...
%
Figure~\ref{fig.mult} shows the event $\Delta$s considering the network latency
...
Nevertheless, the higher are the deadlines, the less applications suit the
middleware.
%
