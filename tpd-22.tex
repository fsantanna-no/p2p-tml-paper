...
In this section, we simulate an application varying the network topology,
latency, and churn, as well as the frequency and deadlines of events.
We use a single machine .
We then evaluate if our design meets its goals as follows:
%
\begin{enumerate}
\item \textbf{Event Deadlines:}
We measure the recurrence of time travels among peers.
If a peer experiences a time travel, it means that it received an event after
its deadline.
For instance, that would be the case when the source peer generates an event
with a small deadline in comparison to the latency to the farthest destination
peer.
More concretely, we teak the middleware implementation to count time travel
ticks.
We then measure the percentage of the sum of all time travels ticks over the to
the total simulation ticks (e.g., $1\%$, if all peers sum $1000$ time travel
ticks over $100k$ accumulated simulation ticks).
%
\item \textbf{Time Synchronization:}
We measure if peers advance in sync in real time using as a reference the peer
most advanced in time.
We start each peer with a random delay to force a time mismatch, and then
verify if they synchronize and remain synchronized during the whole simulation.
More concretely, we teak the middleware to .
...
\end{enumerate}
 ...
 to ensure that all
peers (a) meet event deadlines, (b) advance in sync in real time, and (c) can
leave and join the network and remain symmetric.
%
For each of these goals, we measure (a) the recurrence of time travels, (b) the
real-time pace of peers, and (c) the correctness of unstable peers.


, respectively.

, such that
they validate goals (a), (b), and (c), respectively.

For each of these goals, we simulate different network scenarios varying the
(i) topology, (j) latency, (k) churn, and (k) frequency and (m) deadlines of
events.
For each scenario, we measure (x) the recurrence of time travels, (y) the
real-time pace of peers, and (z) the correctness of unstable peers, such that
they validate goals (a), (b), and (c), respectively.
