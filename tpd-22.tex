...
\IEEEPARstart{R}{eal-time} networked applications allow multiple users to
interact remotely and yet share the same experience.
...
%
We perform simulation varying the network latency and peer churn, as well as
...
We then count all time travel occurrences in the evaluation.
%
...
We then count the mismatches between the clocks in the evaluation.
%
Regarding (c), we make peers leave and join the network periodically, possibly
creating partitions.
We then assert that they all catch up in real time within a short period in the
evaluation.
...
For instance, a \code{50ms} latency with a \code{2x} multiplier results in
\code{500ms} event delays, i.e., each event is triggered with this delay to
...
\begin{figure*}[t]
  ...
  \caption{
    \label{fig.baks}
The \code{5x}, \code{2x}, and \code{1x} delay regions each contains $36$
measures for multiple configurations of event rate (\code{5-200evt/min} and
network latency (\code{5-500ms}).
%
...
The colors indicate the feasibility of each tested scenario
    ...
    \emph{black=faulty}).
%
For instance, the three framed rectangles ($0.1$, $3.1$, and $7.2$) represent
the scenarios with $50ms$ network delay, generating $25evt/min$.
...
much time the recovering peers take to catch up in real time:
...
Regardless of the network latency, event rate, and delay multiplier,
    the variation in the pace of peers is under $0.2\%$ (goal b), and
    recovering peers take $1.3$ seconds to catch up in time (goal c).
Regarding goal (c), note that the middleware is configured to make small steps
towards smooth time transitions, which take $1$ second to complete.
Therefore, the average of $1.3$ seconds shows that peers need no more than 2
time travels while recovering.
