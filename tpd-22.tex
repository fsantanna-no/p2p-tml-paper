...
In this section, we evaluate if our design meets its goals to ensure that all
...
We can count all time travel occurrences during the simulation.
%
Regarding (b), we can use an absolute simulation clock to track, in real time,
the pace of each peer in comparison to a reference peer most advanced in time.
%
Regarding (c), we can TODO specific peers and very that they TODO within a
...
Figure~\ref{fig.topo}, with straight lines, long cycles, and short cycles.
%
In order to have an absolute clock to compare peers, we use a single machine
to simulate all peers, and output their logs to a single file.
We TODO the network latency and clock skew using TODO and TODO.

We now detail why we measure the properties above and how we instrument the
middleware implementation:
%
\begin{enumerate}
%
\item \textbf{Recurrence of time travels:}
We teak the middleware implementation to count time travel ticks.
