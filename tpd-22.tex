...
It first (and last) calls \code{cb\_ini} \lin{2,41} to initialize (and
finalize) the application.
Then, it calls \code{cb\_sim} \lin{3}, passing the event \code{START} to start
...
%
If we receive a time synchronization packet \code{SYNC} from the future
\lin{7--10}, then the we fast forward the application to catch up in time
...
We detail the function \code{p2p\_travel} further
