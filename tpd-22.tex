...
To achieve this, the loop continuously checks for network packets, local
inputs, and time ticks, as follows:

{\footnotesize
\begin{verbatim}
01  void p2p_loop (...,cb_ini,cb_sim,cb_ren,cb_evt) {
01    cb_ini(1);
01    cb_sim(<P2P_EVT_INIT>);
01
01    while (<app-running>) {
01      if (<next-packet>) {
01        // fast forward if too late
01        if (<packet-sync-time-future>) {
01          for (<step-by-step-sleep>) {
01            p2p_travel(<packet-sync-time>);
01            cb_ren();
01          }
01        }
01
01        if (<packet-evt>) {
01          if (<packet-evt-time-past>) {
01            for (<step-by-step-sleep>) {
01              p2p_travel(<packet-evt-time>);
01              cb_ren();
01            }
01            for (<step-by-step-sleep>) {
01              p2p_travel(<now>);
...
01        p2p_evt evt;
01        if (cb_rec(&evt)) {
01          p2p_bcast(<future>, &evt);
01        }
01      }
01
01      if (<next-tick>) {
01        cb_sim(<P2P_EVT_TICK>);
01        <take-snapshot-every-XXXms>
01        cb_ren();
01      }
01    }
01
