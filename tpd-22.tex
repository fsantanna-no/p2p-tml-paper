...
rollback conflicting nodes to resynchronize their state.
%
...
machine can rollback nodes to a previous state and reapply events in order and
in time.
...
API in which developers need to conform.
%
We guide our discussion through a didactic example in which multiple users use
the arrow keys to change the direction of a moving rectangle.

- same keys, same behavior, pause at the same time
